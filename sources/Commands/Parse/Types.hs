...
-- >>> let dictation = SensitiveParser $ \context -> Dictation <$> anyWord `manyUntil` context
...
{- |  the Alternative laws are like the Monoid laws (I guess?):
...
(SensitiveParser $ \\s  ->  let w'' = w' s in  let v'' = v' (Some w'') in  u' (Some v'')  \<*>  (v'' \<*> w'')


two parallel lambdas, applied to the same argument?

{{u' (Some vt)  \<*>  (vt \<*> ws)}}  clearly doesn't equal  {{u' (Some vt)  \<*>  vt}} :-(
but that's okay I think, because then {{u' (Some vt)  \<*>  vt}} would equal only {{SensitiveParser u' \<*> SensitiveParser vt}}
...
-- where did the s go off to? they must be the same.
 contract: applySensitiveParser
(SensitiveParser $ \\s -> (u' (Some (v' (Some (w' s)))) \<*> (SensitiveParser v' \<*> SensitiveParser w')


 beta: let v''
(SensitiveParser $ \\s -> let v'' = v' (Some (w' s)) in u' (Some v'') \<*> v''





SensitiveParser satisfies /homomorphism/:
...
SensitiveParser satisfies /interchange/:
...
given a lemma for SensitiveParser:

[/left-pure/]

     pure f \<*> SensitiveParser x = (SensitiveParser $ \\q -> pure f \<*> x q)

a proof of the /left-pure/ lemma:
